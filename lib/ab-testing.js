...
module.exports = {
  createTest: function(name, config) {
    ...
    var test = function(testGroupName, testConfig, that) {
      if (typeof testGroupName !== "string")
        throw Error("Test Group is not a string");

      if (!_.isArray(testConfig))
        throw Error("Introduce an array of testConfig as a second parameter");

      if (testConfig.length !== testData.length)
        throw Error(
          "The number of testConfig does not match the number of test groups"
        );
      ...
      return testConfig[index].outcome;
    };
